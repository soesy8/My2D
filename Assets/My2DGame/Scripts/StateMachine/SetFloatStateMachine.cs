...
{
    /// <summary>
    /// StateMachine에서 애니메이션 float형 파라미터 값을 세팅하는 클래스
    /// StateMachineBehaviour를 상속받은 클래스는 애니메이터의 상태, 상태머신에 부착되어 신행된다
    /// </summary>
    public class SetFloatStateMachine : StateMachineBehaviour
    {
        /*private void OnEnable() 버그 개 쒸발럼
        {
            Debug.Log("Behaviour Loaded");
        }*/

        #region Variables
        public string floatName;         //bool형 파라미터 이름

        public float enterValue;         //상태머신 들어올 때 bool형 값
        public float exitValue;          //상태머신 나갈 때 bool형 값
        #endregion
